...
class MyRedis extends RedisBase
{
    ...
    private        $k;
    //当前地址
    protected static $host = '1270.0.0.1';
    //当前端口
    protected static $port = 6379;
    //当前权限认证码
    protected static $auth = '';
    //key前缀
    protected static $prefix = 'stats_';
    //当前数据库
    private $select = 0;
    //连接属性数组
    protected $attr = [
    ...
    /**
     * 连接数据库
     * @param array $config [host,port,auth]
    ...
    private function connect($config = [], $attr = [])
    {
        if (extension_loaded('redis')) {
            ...
            self::$auth       = $config['auth'] ?? (self::$redisCnf['auth'] ?? '');
            $this->redis      = new \Redis();
            if (isset($this->attr['persistent']) && $this->attr['persistent']) {
                $persistentId = 'persistent_id_' . ($this->attr['select'] ?? 0);
